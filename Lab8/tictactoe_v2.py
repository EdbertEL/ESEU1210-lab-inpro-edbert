...
class TicTacToeGame:
    def __init__(self, board_size, players=DEFAULT_PLAYERS):
        ...
        self.win_combo = simpledialog.askinteger(
             ...
             prompt="Masukkan angka antara 3 dan 9:",
             minvalue=3,
             maxvalue=9)
        self._current_moves = []
...
def main():
    def start_game():
        try:
            ...
            if board_size < 3:
                raise ValueError("The board size must be 3 or larger.")
            if board_size > 9:
                raise ValueError("The board size must be 9 or smaller.")
            root.destroy()
    ...
    tk.Label(root, text="Enter board size: (From 3 - 9)", font=font.Font(size=14)).pack(pady=10)
    entry = tk.Entry(root, font=font.Font(size=14))
    entry.pack(pady=10)

    tk.Button(root, text="Start Game", command=start_game, font=font.Font(size=14)).pack(pady=20)
    error_label = tk.Label(root, text="", fg="red", font=font.Font(size=14))
    error_label.pack()
